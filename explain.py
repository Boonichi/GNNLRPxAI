...
from xgcn.xgraph import Pad, XSample, XGraph, XNode
...
def occlude(graph, jsonl, xgcn, adjacency, embedding, drop, step, padding, verbose, line_counter):
    ...
    for ratio in np.arange(0., (drop + step), step=step):
        ...
        if verbose:
            logging.info(f"Dropped {dropped_edges} weights of {len(edge_weights_and_positions)} at ratio {ratio}")
        ...
        with open(path_out, 'w+') as fout:
            for line in tqdm(fin.readlines()):
                ...
                if 0 <= crop <= line_counter:
                    logging.info(f"Terminating after {line_counter} lines, due to crop of {crop}.")
                    break
                ...
                label, graph = line_to_graph(line=line, nlp=nlp)
